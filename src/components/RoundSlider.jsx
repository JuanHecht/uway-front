...
function RoundSlider() {
  const [sliderValue, setSliderValue] = useState(50);
  const { getMoodAndImageUrl } = useContext(AuthContext);
  const [moodAndImageUrl, setMoodAndImageUrl] = useState({ mood: '', imageUrl: '' });

  useEffect(() => {
    const updateMoodAndImageUrl = () => {
      const { mood, imageUrl } = getMoodAndImageUrl(sliderValue);
      setMoodAndImageUrl({ mood, imageUrl });
    };
    updateMoodAndImageUrl();
